...
\begin{equation}\label{eqn:cont_Z}
  Z = \int \displaystyle\prod_x \; d\phi(x) \; e^{-S[\phi]}.
\end{equation}
...
\begin{equation}\label{eqn:cont_latt_dic}
  \begin{array}{rcl}
    ...
    \partial_{\mu}\phi(x) & \longrightarrow & \Delta^f_{\mu} = \frac{\phi(na + \hat{\mu}a) - \phi(na)}{a} \,, \\
    \partial_{\mu}\phi(x) & \longrightarrow & \Delta^b_{\mu} = \frac{\phi(na) - \phi(na-\hat{\mu}a)}{a} \,, \\
    \partial_{\mu}\phi(x) & \longrightarrow & \Delta^s_{\mu} = \frac{\phi(na + \hat{\mu}a) - \phi(na-\hat{\mu}a)}{2a} \,, \\
    \mathcal{D}\phi & \longrightarrow & \displaystyle\prod_n \int d\phi(n a).
  \end{array}
\end{equation}

We can immediately see from the Eq.~\eqref{eqn:cont_latt_dic}, that there are many choices of constructing the lattice Laplacian operator $\Box_L$ - we can use the forward difference operators, the backward difference operators, the symmetric or a combination of any two of these. Since we are in any case summing over all spacetime points when computing the action, and we have periodic boundary conditions - some of the combinations will give the same result. For this lecture we choose the conventional choice of,
\begin{equation}\label{eqn:laplatt}
  \Box_L \hat{\phi}(na) = \Delta^b_{\mu}\Delta^f_{\mu} = \frac{1}{a^2} \displaystyle\sum_{\mu} \left(\phi(na + \hat{\mu}a) + \phi(na - \hat{\mu}a) - 2\phi(na) \right) \,.
\end{equation}
In order to write down the full lattice action for the scalar field theory, we need to consider the dimensionality of the lattice parameters like the field value $\phi(na)$ and the lattice mass $M$. We need to maintain the dimensionlessness of the Euclidean lattice action. Looking at Eq.~\eqref{eqn:action} and Eq.~\eqref{eqn:laplatt}, we get \sideex{There is an exercise related to this!}
\begin{equation}\label{eqn:dimless_Mphi}
  ...
  \end{array}
\end{equation}
defining the dimensionless lattice quantities with hats and removing the label $L$ from the box operator. Using this one can re-write the Euclidean lattice action as \sidefact{The factor $2d$ ($= 8$ in 4 dimensions) comes from the diagonal part of $-\hat{\Box}$, summing over all directions.},
...
\begin{equation}\label{eqn:gen_func}
  Z[J] = \int \displaystyle \prod_{l} d\phi_l \; \exp\left({-\frac{1}{2} \displaystyle\sum_{n,m} \hat{\phi}_n K_{nm} \hat{\phi}_m + J_n \phi_n }\right)\,.
\end{equation}
...
\begin{equation}\label{eqn:int_gen_func}
  Z[J] = \frac{1}{\sqrt{K}} \exp\left({\displaystyle\sum_{n,m} J_m K^{-1}_{m,n} J_n }\right)
\end{equation}
...
\begin{equation}
  \langle \hat{\phi}_n \hat{\phi}_m \rangle = \frac{\delta Z[J]}{\delta J_n \delta J_m}|_{J=0} = K^{-1}_{m,n}
\end{equation} 
